...
**Description:** Continuously follows a moving target using the selected navigation method.

**Fail Condition:** Chase continuously tracks a Transform or GameObject target and updates the agent’s destination every frame. Unlike MoveTo, this node reacts to target movement in real time — if the target moves, the agent adjusts its path accordingly.

The movement behavior depends on the selected Navigation Type:
• None — Direct movement without a navigation system. Speed, rotation speed, stopping distance, and movement plane (XZ / XY / XYZ) are controlled by this node. The target position is updated every frame.
• NavMesh — Uses Unity NavMeshAgent. The agent is continuously fed updated target positions. Speed, rotation, and stopping distance are controlled by the NavMeshAgent component.
• Nav3D — Uses Nav3D FollowTarget for volumetric navigation. Requires Nav3DInitializer in the scene and runs only in Play Mode.

This node is ideal for behaviors such as chasing enemies, following allies, escorting units, or tracking any moving target.

**Notes:** Returns Running while the agent is chasing the target.
Returns Success when the agent reaches the stopping distance to the target.
Returns Failure if the target becomes invalid or required components are missing.

If you need a one-time movement toward a fixed position, use the MoveTo node instead.
...
**Description:** Continuously moves the agent away from a target using the selected navigation method.

**Fail Condition:** Flee causes the agent to actively escape from a specified target by continuously recalculating a movement direction opposite to the threat. Unlike MoveTo, Flee does not move toward a fixed position — instead, it dynamically updates the destination every frame based on the target’s current position.

The agent will keep fleeing until the distance to the target is greater than or equal to the configured Flee Distance.

The movement behavior depends on the selected Navigation Type:
• None — Direct movement without a navigation system. Speed, rotation speed, flee distance, and movement plane (XZ / XY / XYZ) are controlled by this node.
• NavMesh — Uses Unity NavMeshAgent to escape along the NavMesh. Path validity, distance progress, and direction changes are monitored to avoid getting stuck or fleeing in the wrong direction.
• Nav3D — Uses Nav3D agent for volumetric navigation. The flee direction is continuously validated, and movement is reissued when the threat changes position or blocks the escape path.

This node is ideal for fear, avoidance, retreat, and evasive behaviors where the agent must react in real time to a moving threat.

**Notes:** Returns Running while the agent is actively fleeing.
Returns Success when the agent reaches a safe distance from the target.
Returns Failure if required parameters or navigation components are missing or invalid.

If you need to move toward a target, use MoveTo or Chase instead.
...
**Description:** Checks whether there is a clear line of sight from the Executor to a resolved Target using raycasts.

**Fail Condition:** Fails if the Target cannot be resolved, if the raycast is blocked by a non-ignored collider, or if the Target is never reached.

**Notes:** • The Target is resolved dynamically via EntryParameter each update.
• Colliders on parent objects can be ignored to prevent self-blocking.
• Colliders with ignored tags are skipped during ray traversal.
• The first non-ignored hit determines the result.
• Uses bounded ray traversal to remain safe for runtime use.
...
**Fail Condition:** Supports multiple materials. Applies new materials to all child Renderers. Returns Success if materials are applied, Failure if no Renderers found.
...
**Description:** Moves the agent toward a fixed target position using the selected navigation method.

**Fail Condition:** MoveTo captures the target position once when the node starts and moves the agent toward that position until the stopping distance is reached. Unlike Chase, this node does not continuously track a moving target — if the target moves after the node starts, the agent will continue toward the original position.

The movement behavior depends on the selected Navigation Type:
• None — Direct movement without a navigation system. Speed, rotation speed, stopping distance, and movement plane (XZ / XY / XYZ) are controlled by this node.
• NavMesh — Uses Unity NavMeshAgent. Speed, rotation, and stopping distance are controlled by the agent component.
• Nav3D — Uses Nav3D agent for volumetric navigation. Requires Nav3DInitializer in the scene and runs only in Play Mode.

This node is ideal for one-time movement orders such as moving to a point, waypoint navigation, or positioning behaviors.

**Notes:** Returns Running while the agent is moving.
Returns Success when the agent reaches the stopping distance.
Returns Failure if required parameters or components are missing or invalid.

If you need continuous tracking of a moving target, use the Chase node instead.
...
**Legacy:** Try to use Chase: ActionNode instead
...
**Legacy:** Try to use Flee: ActionNode instead
...
**Legacy:** Try to use MoveTo: ActionNode instead
...
**Legacy:** Try to use Patrol: ActionNode instead
...
**Description:** Patrols through waypoints using the selected navigation method.

**Fail Condition:** Patrol moves the agent through Blackboard.Waypoints sequentially. Supports looping, inversion, and nearest-start selection.

Navigation Types:
• None — Manual transform movement.
• NavMesh — Uses NavMeshAgent.
• Nav3D — Uses Nav3D MoveTo per waypoint.



**Notes:** Returns Running while patrolling.
Returns Success when patrol finishes (if not endless).
...
**Description:** Generates a random destination around a given origin and validates its reachability using the selected navigation system.

**Fail Condition:** RandomDestination generates a random point within the configured Range around the provided Origin (Transform, GameObject, or Vector3). Depending on the selected NavigationType, the node optionally validates whether the generated point is reachable:

• None — no validation is performed.
• NavMesh — validates reachability using NavMesh path calculation.
• Nav3D — asynchronously validates reachability using Nav3D pathfinding.

When a valid destination is found, it is stored in the configured SaveParameters.

**Notes:** • Returns Success when a valid destination is generated and stored.
• Returns Running while waiting for Nav3D async pathfinding to complete.
• Returns Failure if the origin is invalid or the destination is unreachable.

Nav3D requires Nav3DInitializer to be present and initialized at runtime.
...
**Description:** Stops the agent immediately using the selected navigation method.

**Fail Condition:** Stop immediately halts the active navigation system and clears any ongoing movement request.

Navigation Types:
• None — No action is performed.
• NavMesh — Stops the NavMeshAgent and clears its current path.
• Nav3D — Invokes the Nav3D agent Stop method.

This node is typically used to cancel active movement behaviors such as MoveTo, Chase, Patrol, or Flee.

**Notes:** Returns Success instantly when the stop operation is executed.
Returns Failure if required navigation components are missing or not properly initialized.
...
**Description:** Controls the character's sprint state using Stamina value

**Fail Condition:** Checks RemainingStamina value to enable or disable sprinting. Integrates with Starter Assets ThirdPerson system. Returns child state if the enabled successfully, and Failure if setup is invalid or dependencies are missing.

**Notes:** Uses EntryParameterBool to determine Stamina Recovery Rate Per Second, Drain Rate Per Second, Sprint Restart Treshold. Allows cooperation with other TPC_StaminaSprint by using Sync with Balckboard. If Sync with Blackboard enabled, RemainingStamina and StopTime will be recovered and stored in Blackboard.  TPC_StaminaSprint will store it's Stop Time during OnStop, and during OnStart will recover stamina according to passed time value. Requires Starter Assets ThirdPerson to be installed.
